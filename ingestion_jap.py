...
# Constants
STATIC_DATA_DIR = Path(os.getenv("DATA_JAP", "/app/static/data/legal_japanese"))  # Use DATA_ENG from .env
STATIC_DATA_DIR.mkdir(parents=True, exist_ok=True)

# Prompt Formatting for Japanese Assistant
...
def messages_to_prompt(messages):
    ...
    if not prompt.startswith("<|im_start|>system"):
        prompt = "<|im_start|>system\nあなたはAlibaba Cloudによって作成されたQwenです。あなたは日本語でユーザーをサポートするマルチリンガルエージェントです。<|im_end|>\n" + prompt
    return prompt + "<|im_start|>assistant\n"

# LLM and Embeddings
Settings.llm = Ollama(
    model="qwen2.5:3b-instruct", # Change LLM here!
    request_timeout=500,
...
data_folder = os.getenv("DATA_JAP", "./data/legal_japanese") 
